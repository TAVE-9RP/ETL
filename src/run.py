...
def run():
    ...
    df_prev_project = get_csv_by_date("project", prev_month_end_str) # 전월 프로젝트 스냅샷
    df_logistics = get_csv_by_date("logistics", target_date_str)
    df_logistics_item = get_csv_by_date("logistics_item", target_date_str)
    df_prev_logistics = get_csv_by_date("logistics", prev_month_end_str) # 전월 말 스냅샷
    ...
    # 3. KPI 분석
    safety_results = calculate_safety_stock_rate(df_project, df_logistics, df_logistics_item, df_item)
    lead_time_results = calculate_shipment_lead_time(df_project, df_logistics)
